...
pub struct SerialPhysicalLayer {
    ...
    _data_rx: std::sync::Mutex<broadcast::Receiver<DataEvent>>,
    _write_rx: std::sync::Mutex<broadcast::Receiver<Vec<u8>>>,
    _error_rx: std::sync::Mutex<broadcast::Receiver<ModbusError>>,
    _connection_close_rx: std::sync::Mutex<broadcast::Receiver<ConnectionId>>,
...
impl SerialPhysicalLayer {
    pub fn new(path: String, baud_rate: u32) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            _write_rx: std::sync::Mutex::new(write_rx),
            _error_rx: std::sync::Mutex::new(error_rx),
            _connection_close_rx: std::sync::Mutex::new(connection_close_rx),
